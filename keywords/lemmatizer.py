...
def process_keywords():
    ...
    # Split lemmatized text into words
    split_words = df['lem_text'].str.split()
    
    # Flatten list & split words into individual words
    all_words = [word for phrase in split_words for word in phrase]
    
    # Remove duplicates & sort words
    remove_duplicates = sorted(set(all_words))
    
    # Create DataFrame
    return pd.DataFrame(remove_duplicates, columns=['keyword'])
...
def lemmatize_input(text):
    ...
    # Tokenize and make to lowercase
    tokens = word_tokenize(str(text).lower())

    if len(tokens) == 1:
        return lemmatizer.lemmatize(tokens[0]) # If single token, lemmatize directly
    else:
...
if not os.path.exists(output_folder):
    os.makedirs(output_folder) # Ensure output directory exist
output_file_path = os.path.join(output_folder, 'lemmatized_key.csv')
...
def lemmatize_phrase(phrase):
    tokens = word_tokenize(str(phrase).lower()) # Tokenize and convert lowercase for each phrase
    tagged = nltk.pos_tag(tokens) # Get POS tags for each token
    lemmatized_tokens = [lemmatizer.lemmatize(word, get_wordnet_pos(tag)) for word, tag in tagged] # Unpack (token, pos_tag) from tagged
    return ' '.join(lemmatized_tokens) # Join back into a single string

# Apply lemmatization to dataframe
df['lem_text'] = df['unlem_text'].apply(lemmatize_phrase) # Apply lemmatization function to each row in 'unlem_text' column
